...
const WelcomeScreen: React.FC<Props> = ({ blocks, onStartBlock, onBack, theme = 'emerald' }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-20 relative z-10">
      ...
      >
        <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:border-${colorClass}-500/30 group-hover:bg-${colorClass}-500/10 transition-all`}>
      ...
      <header className="mb-10 md:mb-16 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border border-${colorClass}-500/30 bg-${colorClass}-500/5 text-${colorClass}-400 text-[10px] uppercase tracking-widest mb-4 md:mb-6 backdrop-blur-sm font-mono`}>
        ...
        <p className="text-gray-400 max-w-2xl text-sm md:text-lg font-light leading-relaxed">
          Ověřte své znalosti v interaktivních simulacích. Každý blok obsahuje sadu otázek zaměřených na konkrétní problematiku.
        </p>
      </header>
        ...
        {blocks.map((block, index) => {
           ...
           return (
            ...
            >
              ...
              <div className={`relative h-full bg-[#0a0a0a]/60 backdrop-blur-xl border border-white/5 rounded-2xl p-6 md:p-8 overflow-hidden hover:bg-[#0a0a0a]/80 transition-all duration-300 group-hover:border-${colorClass}-500/30 hover:-translate-y-1`}>
                ...
                <div className="flex justify-between items-start mb-4 md:mb-6">
                    <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl bg-${colorClass}-950/20 border border-${colorClass}-500/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-500`}>
                         <IconComponent className={`w-5 h-5 md:w-6 md:h-6 text-${colorClass}-400`} />
                    </div>
                    <div className={`text-[10px] font-mono text-gray-600 border border-white/5 px-2 py-1 rounded bg-black/40 group-hover:text-${colorClass}-300 transition-colors`}>
                        BLOK {block.id.toString().padStart(2, '0')}
                    </div>
                </div>

                <h3 className="text-lg md:text-xl font-bold text-white mb-2 md:mb-3 group-hover:text-white font-display uppercase transition-colors">
                ...
                <div className={`mt-auto flex items-center gap-2 text-[10px] md:text-[11px] font-bold text-${colorClass}-500/70 group-hover:text-${colorClass}-400 uppercase tracking-widest transition-colors font-mono`}>
